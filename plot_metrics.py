...
def get_args():
    ...
    parser.add_argument('--metric', type=str, required=True, help='SSIM or PSNR', choices=['SSIM', 'PSNR'])
...
def main():
    ...
    plt.xlabel("R")
    plt.ylabel("Masked " + args.metric if args.masked else args.metric)
    plt.xticks(R_values)
    ...
    for steps in steps_values:
        ...
        plt.plot(R_values, means, '--o', color=step_color[steps], linewidth=1.2, label=f'{steps} steps (DDIM)')
